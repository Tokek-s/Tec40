  ...
  <div class="min-h-screen flex bg-slate-100">
    ...
    <aside class="hidden lg:flex w-72 bg-[#B50505] text-white flex-col">
      <div class="p-6 border-b border-white/10">
        <Link :href="route('prefectos.dashboard')" class="flex items-center space-x-3 hover:opacity-80 transition-opacity cursor-pointer">
          <img src="/images/logo-escuela.png" class="h-10 w-10 object-contain" alt="Logo" />
          <div>
            <p class="text-sm text-white/80">Prefectura</p>
          </div>
        </Link>
      ...
      <nav class="flex-1 overflow-y-auto p-3 space-y-1">
        <Link :href="route('prefectos.dashboard')" :class="navItemClass(route().current('prefectos.dashboard'))">
          <span class="i">🏠</span>
        ...
        <div class="mb-1">
          ...
          <div v-if="open.alumnos" class="mt-1 ml-4 space-y-1">
            <Link :href="route('prefectos.alumnos.pase_lista')" :class="subItemClass(route().current('prefectos.alumnos.pase_lista'))">
              <span class="pr-2">›</span> Pase de Lista
            </Link>
            <Link :href="route('prefectos.alumnos.lista')" :class="subItemClass(route().current('prefectos.alumnos.lista'))">
          ...
          <aside class="fixed top-0 left-0 bottom-0 w-80 bg-[#B50505] text-white flex flex-col shadow-xl">
            <div class="p-6 border-b border-white/10">
              <div class="flex items-center justify-between mb-4">
                <Link :href="route('prefectos.dashboard')" class="flex items-center space-x-3 hover:opacity-80 transition-opacity cursor-pointer" @click="mobileMenuOpen = false">
                  <img src="/images/logo-escuela.png" class="h-10 w-10 object-contain" alt="Logo" />
                  <div>
                    <p class="text-sm text-white/80">Prefectura</p>
                  </div>
            ...
            <nav class="flex-1 overflow-y-auto p-3 space-y-1">
              <Link :href="route('prefectos.dashboard')" :class="navItemClass(route().current('prefectos.dashboard'))" @click="mobileMenuOpen = false">
                <span class="i">🏠</span>
                <span>Información General</span>
              </Link>
              <div class="mb-1">
                ...
                <div v-if="open.alumnos" class="mt-1 ml-4 space-y-1">
                  <Link :href="route('prefectos.alumnos.pase_lista')" :class="subItemClass(route().current('prefectos.alumnos.pase_lista'))" @click="mobileMenuOpen = false">
                    <span class="pr-2">›</span> Pase de Lista
                  </Link>
                  <Link :href="route('prefectos.alumnos.lista')" :class="subItemClass(route().current('prefectos.alumnos.lista'))" @click="mobileMenuOpen = false">
                    <span class="pr-2">›</span> Lista Alumnos
                  </Link>
                </div>
              </div>
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <header class="lg:hidden bg-white shadow-sm px-4 py-3 flex items-center justify-between sticky top-0 z-40">
        ...
        <div class="flex items-center space-x-2">
          ...
          <span class="text-sm font-semibold text-gray-700">Prefectura</span>
        </div>
...
const open = reactive({ alumnos: true })
